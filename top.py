...
class BooleanAtlas10Ops(wiring.Component):
    """
    Full GenesisAI-compatible pipeline in hardware.

    Input: packed N nibbles (4*N bits)
    Mode:
      - K=2 : pol2  => for each pair, apply all 10 ops and count results into 16-bin histogram
      - K=3 : pol3  => for each triple, apply all 10x10 op chains (op2(op1(a,b),c)) and count

    Output: histogram RAM readable at rd_addr (0..15)
    """

    # Operator IDs (must match GenesisAI operator set order you use conceptually)
    OP_AND    = 0
    OP_OR     = 1
    OP_IMPLY  = 2
    OP_CONV   = 3
    ...
    def __init__(self, n_nibbles: int, k: int, count_width: int = 32):
        super().__init__()
        assert n_nibbles >= 1 and n_nibbles <= 32
        assert k in (2, 3), "This core implements pol2 (k=2) and pol3 (k=3)"
        ...
        # Control
        self.start = In(1)   # pulse
        self.busy  = Out(1)
        self.done  = Out(1)  # level high when complete

        # Data: packed N nibbles (little-endian packing is fine as long as sim matches)
        self.data_in = In(4 * n_nibbles)

        # Read histogram
        self.rd_addr = In(4)
        self.rd_data = Out(count_width)

    @staticmethod
    def _apply_op(m: Module, op_sel: Value, a: Value, b: Value, out: Signal):
        """
        GenesisAI-compatible bitwise operators on 4-bit vectors.
    ...
    def elaborate(self, platform):
        ...
        # -------------------------
        # Select K elements from mask (scan)
        # -------------------------
        scan_i = Signal(range(N + 1))
        sel0 = Signal(4)
        sel1 = Signal(4)
        sel2 = Signal(4)  # only used when K=3
        sel_count = Signal(range(4))  # up to 3

        # -------------------------
        # Operator loop counters
        # -------------------------
        op1 = Signal(4)  # 0..9
        op2 = Signal(4)  # 0..9 (only for K=3)

        # combinational op results
        t1 = Signal(4)
        t2 = Signal(4)

        # apply ops
        self._apply_op(m, op1, sel0, sel1, t1)       # t1 = op1(sel0, sel1)
        self._apply_op(m, op2, t1,  sel2, t2)        # t2 = op2(t1, sel2)

        # for K=2, final result is t1; for K=3, final result is t2
        final_res = Signal(4)
        if K == 2:
        ...
        m.d.comb += [self.busy.eq(busy), self.done.eq(done)]
        ...
        with m.FSM(name="atlas10"):
            ...
            # Scan through mask bits to capture sel0/sel1(/sel2)
            with m.State("SCAN_SELECT"):
                with m.If(scan_i < N):
                    with m.If(mask[scan_i]):
                        ...
                        with m.Else():
                            # sel_count == 2 (only meaningful for K=3)
                            m.d.sync += sel2.eq(nibbles[scan_i])
                        m.d.sync += sel_count.eq(sel_count + 1)
                    m.d.sync += scan_i.eq(scan_i + 1)
                with m.Else():
                    # selection complete, go to operator loops
                    m.d.sync += [
                        op1.eq(0),
                        op2.eq(0),
                    ]
                    if K == 2:
                        m.next = "OP2_LOOP_POL2"
                    else:
                        m.next = "OP12_LOOP_POL3"

            # -------------------------
            # pol2: loop op1=0..9
            # -------------------------
            with m.State("OP2_LOOP_POL2"):
                # For each op1, increment histogram[ result(op1(sel0,sel1)) ]
                m.d.sync += [
            ...
            # -------------------------
            # pol3: loop op1=0..9, op2=0..9
            # -------------------------
            with m.State("OP12_LOOP_POL3"):
                m.d.sync += [
                    bin_val.eq(final_res),
                    use_internal_read.eq(1),
                    int_rd_addr.eq(final_res),
            ...
            # Histogram read (sync) then write increment
            with m.State("HIST_READ"):
                m.next = "HIST_WRITE"

            with m.State("HIST_WRITE"):
                m.d.sync += old_count.eq(rd.data)
                m.d.sync += [
                    wr.en.eq(1),
                    wr.addr.eq(bin_val),
            ...
            # Advance op loops or subset mask
            with m.State("LOOP_ADVANCE"):
                m.d.sync += [
                    wr.en.eq(0),
                    use_internal_read.eq(0),
                ]
                ...
                if K == 2:
                    # advance op1
                    with m.If(op1 == 9):
                        # finished all 10 ops for this subset -> next subset
                        m.next = "NEXT_SUBSET"
                    with m.Else():
                        m.d.sync += op1.eq(op1 + 1)
                        m.next = "OP2_LOOP_POL2"
                else:
                    # advance op2 fastest, then op1
                    with m.If(op2 == 9):
                        with m.If(op1 == 9):
                            m.next = "NEXT_SUBSET"
                        with m.Else():
